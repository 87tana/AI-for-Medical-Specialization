...
def check_for_leakage(df1, df2, patient_col):
    ...
    # Extract unique patient IDs from both datasets
    ...
    # If there are common patients, there is leakage
...
def compute_class_freqs(labels):
    ...
    # Count the number of positive labels for each class
    positive_counts = np.sum(labels, axis=0)
    
    # Negative counts for each class are just the total count minus the positive count
    negative_counts = N - positive_counts
    
    # Calculate positive frequencies
    positive_frequencies = positive_counts / N
    
    # Calculate negative frequencies
    negative_frequencies = negative_counts / N
